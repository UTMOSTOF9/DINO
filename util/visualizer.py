...
def fig2numpy(fig, dpi: int = 180):
    '''
    This function is used to transfer fig to numpy image.

    Args:
        fig (Figure): matplotlib's figure
        dpi (int, optional): DPI settings. Defaults to 180.

    Returns:
        img: transferred image.
    '''
    buf = io.BytesIO()
...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=120, savedir=None, savepath=None, show_in_console=True):
        """
        img: tensor(3, H, W)
        ...
        if savepath is not None:
            ...
            if caption is None:
                savepath = '{}/{}-{}.png'.format(savedir, int(tgt['image_id']),
                                                 str(datetime.datetime.now()).replace(' ', '-'))
            else:
                savepath = '{}/{}-{}-{}.png'.format(savedir, caption,
                                                    int(tgt['image_id']), str(datetime.datetime.now()).replace(' ', '-'))
            print("savename: {}".format(savepath))
            os.makedirs(os.path.dirname(savepath), exist_ok=True)

        img = fig2numpy(fig, dpi=dpi)
